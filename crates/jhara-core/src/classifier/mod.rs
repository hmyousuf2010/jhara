...
impl RuleEngine {
    pub fn classify(project: &DetectedProject) -> ClassifiedProject {
        ...
        let staleness_res = checker
            .evaluate(&project.root_path, Some(project.signature_mtime))
            .ok();

        let staleness = staleness_res
            .as_ref()
            .map(|r| if r.is_stale { 1.0 } else { 0.1 })
            .unwrap_or(0.0);
        ...
        let all_safe = project
            .artifacts
            .iter()
            .all(|a| a.safety_tier == SafetyTier::Safe);
        let has_blocked = project
            .artifacts
            .iter()
            .any(|a| a.safety_tier == SafetyTier::Blocked);

        if has_blocked {
            category = ActionCategory::Keep;
            recommendation =
                "Contains protected system files. Not recommended for deletion.".to_string();
        } else if all_safe && total_size > 100 * 1024 * 1024 && staleness > 0.5 {
            category = ActionCategory::QuickWin;
            recommendation = format!(
                "Safe to delete. Reclaims {} MB.",
                total_size / (1024 * 1024)
            );
        } else if has_ghosts {
            category = ActionCategory::GhostCleanup;
            recommendation =
                "Historical traces found. Safe to remove from logs/history.".to_string();
        }
